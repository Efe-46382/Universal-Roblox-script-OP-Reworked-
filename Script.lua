...
Section:NewLabel("Bypass first person lock (f)", "Bypass first person")
...
Section4:NewButton("Scan for kill Remotes", "Scans ReplicatedStorage for events", function()
	...
	if foundRemote then
		...
		StarterGui:SetCore("SendNotification", {
			Title = "Potential remote found!";
			Text = "Backdoor has been found!";
			Duration = 3;
		})
	else
...
Section4:NewButton("Execute Kill", "Fires the detected remote", function()
	local codeToSend = 'game.Players["' .. targetName .. '"].Character.Humanoid.Health = 0'
	...
	if foundRemote then
		foundRemote:FireServer(codeToSend)
		foundRemote:FireServer(targetPlr, math.huge)
		print("Fired code to: " .. foundRemote)
	else
		print("Remote not selected!")
		local StarterGui = game:GetService("StarterGui")
		StarterGui:SetCore("SendNotification", {
			Title = "Remote not selected!";
...
toggleSection:NewKeybind("Bypass first person lock (f)", "Bypass first person", Enum.KeyCode.P, function()
	game.Players.LocalPlayer.CameraMode = Enum.CameraMode.Classic
	print("Bypassed!")
end)
